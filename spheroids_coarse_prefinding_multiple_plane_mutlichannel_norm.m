...
% Ignore the empty wells
row_list = 5:12;
col_list = 5:15;
%%
% path_root = 'G:\Data\IBIN_Nina\20201029_fixedPlate';
path_root = 'E:\IBIN_Nina\20201029_fixedPlate'
path_root = 'U:\IBIN_Nina\20201120_leoTraining'
path_list = {
    [path_root '\LED_430 - Copy'];
    [path_root '\LED_505 - Copy'];
    [path_root '\LED_565 - Copy'];
    [path_root '\LED_625 - Copy'];
    };
...
led_table = readtable([path_root '\led_wavelength_table.xls']);
leds = led_table{:, 'LED'};
dyes = led_table{:, 'dye'};
filters = led_table{:, 'filters'};
...
for i_led = 1:numel(path_list)
    ...
    % replace - with space, then get dir again to sort properly, this is 
    % because micromanager SOMETIMES replaces spaces with hypens in filenames
    replace_hyphens_micromanager(imgs_bottom{i_led}, 'images')
    replace_hyphens_micromanager(imgs_focus{i_led}, 'images')
    replace_hyphens_micromanager(imgs_top{i_led}, 'images')
...
col_array = cell(length(imgs_bottom{1}), 1);
row_array = cell(length(imgs_bottom{1}), 1);
...
% matrix showing wells that are included for given dye (2nd index) [for debug]
...
for j=1:length(imgs_bottom{1})
    ...
    row_array{j} = {row, rowNum};
    col_array{j} = {col, colNum};
...
all_c = zeros(88,1);
for j=1:length(imgs_bottom{1})
    output_data_best(j).filter_list = string([]);
    ...
    for i = 1:numel(path_list) % loop over each LED channel
        ...
        output_data{i}(j).row = row_array{j}{1};
        output_data{i}(j).col = col_array{j}{1};
        ...
        if max_val > max(max_val_array{j}(:))
            ...
            output_data_best(j).row = row_array{j}{1};
            output_data_best(j).col = col_array{j}{1};
            output_data_best(j).max_val = max_val;
            % fullfile([color_path '\all'],imgs_bottom{i}(j).name)
