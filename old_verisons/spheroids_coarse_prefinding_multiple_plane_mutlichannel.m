...
% NORMALISE
det_threshold = 500; % detection threshold (above which all included) for presence of spheroid
...
col_list = 5:15;
%%
path_root = 'Z:\Projects\OPM\Nina_project\prefind\20201008_fixedPlatePrefindTest';
path_list = {
    [path_root '\LED_430'];
    [path_root '\LED_505'];
    [path_root '\LED_565'];
...
for j=1:length(imgs_bottom{1})
    ...
    for i = 1:numel(path_list) % loop over each LED channel
        ...
        % normalise (CHANGE det_threshold)
        img_in_bottom = (2^16-1)*double(img_in_bottom)/double(max(img_in_bottom(:)));
        img_in_focus = (2^16-1)*double(img_in_focus)/double(max(img_in_focus(:)));
        img_in_top = (2^16-1)*double(img_in_top)/double(max(img_in_top(:)));
        ...
        % if plot_and_save_figs
            ...
            mkdir([save_path '\spheroid_median']);
            imwrite(uint16(img_in), fullfile([save_path '\spheroid_median'],['median' imgs_bottom{i}(j).name]))
        % end
