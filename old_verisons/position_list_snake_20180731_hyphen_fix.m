...
file_data_raw = file_data;
file_data = strrep(file_data, 'Row-', 'Row ');
file_data = strrep(file_data, '-Column-', ' Column ');
file_data = strrep(file_data, '-Field-', ' Field ');
...
for row_num = 1:row_num
    ...
    for column_num = col_num
        ...
        new_x = num2str(round(x_pos-x_shift))
        new_y = num2str(round(y_pos-y_shift))
        
        if ~isnan(output_data(file_index).xpos)
            %Create a new string with modified x position
            new_string=strrep(file_data_raw([start_of_entry:end_of_entry]),num2str(x_pos),num2str(new_x));
...
dlmwrite(['U:\IBIN_Nina\20201120_leoTraining\positions_final_pre3.pos'], new_file_data,'delimiter','');
