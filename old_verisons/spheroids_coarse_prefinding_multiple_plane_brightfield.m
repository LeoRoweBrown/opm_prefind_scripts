...
% end
% local min 
local_min = imregionalmax(img_in)*255;
figure(2)
imshow(local_min)
...
% if plot_and_save_figs
    ...
    mkdir([path_root '\spheroid in false-color-data'])
    saveas(gcf, fullfile([path_root '\spheroid in false-color-data'],['false-colour-data' imgs_bottom(j).name]))
    ...
    mkdir([path1 '\spheroid in raw-data'])
    saveas(gcf, fullfile([path1 '\spheroid in raw-data'],['raw-data' imgs_bottom(j).name]))
% end
...
save([path1 '\output_data'] , 'output_data', 'output_data')
