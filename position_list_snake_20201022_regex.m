...
pos_path = 'G:\Data\IBIN_Nina\20201120_leoTraining\positions_prefind.pos'
...
load('G:\Data\IBIN_Nina\20201120_leoTraining\colorPrefind_20201029_5\output_data_best.mat')
...
%% add square brackets for micromanager in devices field
new_file_data = strrep(new_file_data, '"DEVICES":{', '"DEVICES":[{');
z_vals = regexp(new_file_data, '(?<="Z":)[0-9]+', 'match');
z_vals = unique(z_vals);
for z = 1:numel(z_vals)
    new_file_data = strrep(new_file_data, sprintf('"Z":%s}', z_vals{z}), sprintf('"Z":%s}]', z_vals{z}));
end

% add linebreaks after commas, brackets, make human readable
% https://github.com/ybnd/prettyjson.m
new_file_data = prettyjson_micromanager(new_file_data);
...
% dlmwrite(['U:\IBIN_Nina\20201120_leoTraining\positions_final_pre.pos'], new_file_data,'delimiter','');
dlmwrite(['G:\Data\IBIN_Nina\20201120_leoTraining\positions_final_pre_fix.pos'], new_file_data,'delimiter','');
